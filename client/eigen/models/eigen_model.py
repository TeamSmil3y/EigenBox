...
class EigenGeneral(BaseModel):
    ...
    root_domain: Annotated[str, BeforeValidator(domain_validator)] = Field(..., description="Root domain for the Eigen service")
    subdomain: str = Field(..., description="Subdomain for the Eigen service")
    secret_key: Optional[str] = Field(..., description="Secret key to authenticate to root server")

class EigenServices(BaseModel):
    """
    Configuration for services.
    """
    lock_dir: Annotated[Path, BeforeValidator(path_converter)] = Field(..., description="Path to the directory for service locks")
